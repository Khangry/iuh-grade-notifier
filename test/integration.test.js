...
function makeDeps(chiTietRows, rl, phieuThu = []) {
  return {
    login: async () => ({ access_token: 't', sub: '1763538', maMap: 'STUDENT123' }),
    api: {
      ...
      phieuThuTongHop: async () => phieuThu,
    },
  };
}
...
test('phiếu thu mới: id mới xuất hiện → embed 💸 cam, format tiền VND + link hóa đơn', async () => {
  ...
  assert.match(e.description, /9001\.pdf/);
});

test('phiếu thu baseline: phiếu đã có trong old → không báo lại', async () => {
  const phieu = [{ id: 9001, tongTien: 1000, ngayThu: '2026-01-01', maHoaDon: 'h', donViThu: 'd', urlInvoice: null }];
  const snap = (await buildSnapshot({}, makeDeps([{ level3: 'Điểm chữ', value: 'A' }], [], phieu))).snapshot;
  const changed = diff(snap, snap); // không đổi
  assert.equal(changed.filter((c) => c.key.startsWith('phieuthu:')).length, 0);
});
